...
class _DetailBeratMatangScreenState extends State<DetailBeratMatangScreen> {
  ...
  Map<String, dynamic>? _riwayatData_pcs;
  ...
  final List<Map<String, dynamic>> tomatoes = [
    {'id': 'Tomat 1', 'weight': 0.002},
    {'id': 'Tomat 2', 'weight': 0.003},
    {'id': 'Tomat 3', 'weight': 0.0025},
    {'id': 'Tomat 4', 'weight': 0.0018},
    {'id': 'Tomat 5', 'weight': 0.0022},
    {'id': 'Tomat 3', 'weight': 0.0025},
    {'id': 'Tomat 4', 'weight': 0.0018},
    {'id': 'Tomat 5', 'weight': 0.0022},
    {'id': 'Tomat 3', 'weight': 0.0025},
    ...
    {'id': 'Tomat 4', 'weight': 0.0018},
    {'id': 'Tomat 5', 'weight': 0.0022},
    // Add more tomato entries as needed
  ];
          ...
          children: [
            ...
            Expanded(
              child: ListView.builder(
                itemCount: (_riwayatData_pcs?['data'] ?? [])
                    .where((tomato) => tomato['kategori'] == 'Matang')
                    .length, // Safely handle potential nulls
                itemBuilder: (context, index) {
                  // Create a filtered list of tomatoes categorized as "Matang"
                  final filteredTomatoes = (_riwayatData_pcs?['data'] ?? [])
                      .where((tomato) => tomato['kategori'] == 'Matang')
                      .toList(); // Convert to list for indexing

                  // Check if filteredTomatoes is empty to avoid index out of range
                  if (filteredTomatoes.isEmpty) {
                    return Center(
                        child: Text(
                            'No tomatoes available in the "Matang" category.'));
                  }

                  final tomato =
                      filteredTomatoes[index]; // Access the filtered tomato
                  ...
                  return Container(
                    ...
                    child: Row(
                      ...
                      children: [
                        Text(
                          tomato['id_tomat'].toString(), // Tomato ID
                          style: const TextStyle(
                            fontSize: 16,
                            fontWeight: FontWeight.bold,
                          ),
                        ...
                        Text(
                          '${tomato['berat_tomat'].toStringAsFixed(3)} Kg', // Weight of the tomato
                          style: const TextStyle(
                            fontSize: 16,
                          ),
                        ),
